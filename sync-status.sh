#!/usr/bin/env bash
# Claude Code status line that also keeps status.json, and so the showcase page, current.
#
...
#   "statusLine": { "type": "command", "command": "~/claude-code-setup/sync-status.sh" }
#
...
publish() {
    ...
    for attempt in 1 2 3; do
        ...
        if ! git -C "$REPO" cat-file -e "$base:$STATUS_PATH" 2>/dev/null; then
            log "$STATUS_PATH is not on $REMOTE/$BRANCH yet; merge the status.json change first"
            return 1
        fi
